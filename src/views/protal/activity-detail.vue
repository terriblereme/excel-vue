  ...
  <div>
    ...
    <div class="div-center container">
      <div class="breadcrumb">
        ...
        <a @click="clickCategory" >{{ categoryName }}</a>
          <el-icon><arrow-right /></el-icon>
      ...
      <div class="pn">
        <a v-if="pre != null" @click="clickPreNext(pre.id)">
          上一篇:{{ pre.title.slice(0, 15) }}
          <span v-if="pre.title.length > 15">...</span>
        </a>
        <a v-if="next != null" @click="clickPreNext(next.id)">
...
export default {
  ...
  methods: {
    clickPreNext(id) {
      this.load(id);
      this.$router.replace(`/activity/detail/${id}`);
    },
    clickCategory(){
      this.$router.push({path:`/activity`,query:{category:this.data.category}});
    },
    load(id) {
      getDetail(id).then((res) => {
        this.data = res.data;
        this.categoryName = this.$consts.activityCategorys.find(item =>item.id==this.data.category).name;
        //获取上一篇
        getList({
          pageNum: 1,
          pageSize: 1,
